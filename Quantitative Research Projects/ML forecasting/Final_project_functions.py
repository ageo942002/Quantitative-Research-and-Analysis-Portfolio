...
# Additional imports for Deep Learning / LSTM support ===
...
# Core series and returns helpers log transformation
...
def rs(name, est, space, X, y, cv, scoring, refit,
    ...
    """Deterministic randomized search (n_jobs=1 to avoid tie jitter)."""
...
def ema(s: pd.Series, span: int) -> pd.Series:
    """Exponential moving average (Wilder-compatible with adjust=False)."""
...
def rsi(close: pd.Series, n: int = 14) -> pd.Series:
    """Relative Strength Index (classic Wilder RSI)."""
...
def to_daily_bfill(sig_w, daily_index):
    """
    Map a weekly signal to a daily index by resampling to calendar days with
    backward-fill (bfill), then forward-fill onto the trading-day index.
    """
...
def ann_sharpe(daily_ret: pd.Series) -> float:
    """
    Annualized Sharpe from daily returns (log or simple).
    Uses classic mean/std * sqrt(252). Robust to 0/NaN stdev.
    """
...
def plot_and_table(bt_dict, title, th, tx, test_start):
    """Rebase each curve to 1.0 at test_start so B&H is visible and curves are comparable."""
    import matplotlib.pyplot as plt
    rows = {}
...
def _to_simple(logret: pd.Series, name: str = "") -> pd.Series:
    """Convert daily *log* returns to simple returns and ensure a DateTimeIndex."""
...
def _clip_bench(bench_simple: pd.Series,
    ...
    """Clip benchmark simple-returns to a date window (inclusive)."""
...
def build_weekly_features(weekly: pd.DataFrame, H: float = 0.0):
    """
    Build leak-free weekly features + label.
    Inputs
      weekly : DataFrame with columns ['Open','High','Low','Close','Adj Close','Volume'] (weekly bars)
      H      : fixed threshold for next-week log-return label (Up = 1 if r_{t+1} > H)
    Returns
      X      : DataFrame of features (weekly index)
      y      : Series of {0,1} labels (aligned to X)
    Notes
      * All features are shifted by 1 week so they use only past information.
      * Safe if Volume is missing.
    """
...
def hysteresis_from_proba(proba_s: pd.Series, enter: float, exit: float) -> pd.Series:
    """
    proba_s : weekly or daily probabilities in [0,1], indexed by time (already shifted if needed)
    enter   : threshold to switch from 0 → 1 (go long)
    exit    : lower threshold to switch from 1 → 0 (go flat)
              Must satisfy 0 <= exit < enter <= 1
    Returns a {0,1} Series with same index (first value assumes flat unless proba >= enter).
    """
...
def make_timeseries_dataset(X, y, lookback=3, batch_size=64):
    """
    Convert X (2D features) and y (1D labels) into TensorFlow
    time-series dataset for LSTM models.
    
    Parameters:
        X : numpy array or DataFrame (scaled features)
        y : Series or array of binary labels (aligned to X)
        lookback : number of past weeks to use as input window
        batch_size : number of samples per training batch

    Returns:
        tf.data.Dataset for model.fit()
    """
...
def create_lstm_model(n_features, lookback=3, lr=0.001):
    """
    Create and compile a standard LSTM binary classifier
...
def evaluate_classification(y_true, y_pred, y_proba=None):
    """
    Compute classification metrics for any model (ML or LSTM).
    """
    metrics = {
...
def plot_metric_comparison(ml_scores, lstm_scores):
    """
    Bar chart comparing metrics (AUC, Precision, Recall, Accuracy)
    for ML vs LSTM models.
    """
